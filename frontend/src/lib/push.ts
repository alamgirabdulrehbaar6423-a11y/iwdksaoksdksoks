...
function base64UrlToUint8Array(value: string): Uint8Array<ArrayBuffer> {
  ...
  // Explicit ArrayBuffer backing keeps this assignable to BufferSource
  // (applicationServerKey) under TS 5.7+ typed-array generics.
...
// ── Missed wake-up ring → normal web push to Faizan (fire-and-forget) ────────
// Reuses the EXISTING send-push-notification Edge Function (untouched). The
// message_id is derived from the wake-up request UUID so push_log dedupes it
// server-side even if both the local timer and a Realtime 'expired' event try
// to send it. NEVER throws.
export async function notifyMissedWakeUp(requestId?: string | null): Promise<void> {
  try {
    if (!isSupabaseConfigured || !VAPID_PUBLIC_KEY) return;
    await supabase.functions.invoke("send-push-notification", {
      body: {
        message_id: `wakeup-missed-${requestId ?? Date.now()}`,
        sender: "habiba",
        recipient: "faizan",
        text: "📞 Missed wake-up ring! I tried to wake you up… 💗",
        type: "text",
        url: "/",
        exclude_session_id: getSessionId(),
      },
    ...
    console.warn("[push] notifyMissedWakeUp failed (non-fatal)", err);
  }
}
