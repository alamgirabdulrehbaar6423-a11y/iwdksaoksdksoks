...
frontend:
  ...
  - task: "NEW THEME SYSTEM: 6 romantic themes + gallery with live preview, static CSS-only wallpapers, reduce-effects mode, persistence"
    implemented: true
    working: true
    file: "src/lib/chat-themes.ts, src/pages/homepage/_components/theme-gallery.tsx, src/pages/homepage/_components/chat-overlay.tsx, src/index.css"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        -working: true
        -agent: "main"
        -comment: "Performance-first theme system implemented per user's strict requirements. ARCHITECTURE: (1) src/lib/chat-themes.ts registry (8 themes: light, dark + 6 new: love, glossy-hearts, pastel-hearts, midnight-love, rose, minimal-love). (2) All theming is pure static CSS in index.css: per-theme design-token variable blocks + ONE generic !important override rule-set scoped to [data-hb-custom-theme]. Applying a theme = flipping data-hb-chat-theme attribute on .hb-chat-container -> single repaint, ZERO ongoing JS/GPU cost. (3) Wallpapers are tiny inline SVG data-URI tiles (~1-2KB each, hearts/stars/sparkles with gloss baked in via SVG radial gradients — NO filters, NO animation, NO network requests); browser rasterizes once and caches. Entire prod CSS incl. all 6 wallpapers = 9.4KB gzip. (4) darkMode boolean replaced by themeId state (localStorage hb_chat_theme, read synchronously at mount -> no flash; migrates old hb_chat_dark_mode key and keeps it in sync); darkMode now derived from theme registry so all existing dark-aware modals/menus/music page work unchanged. (5) Header sun/moon toggle replaced by palette button (data-testid=chat-theme-btn) opening ThemeGallery bottom sheet (no framer-motion, one-shot CSS animations only, renders null when closed): 2-col grid of live swatch cards (pure CSS pseudo-element mini-bubbles, same tokens as real theme), tap -> full-screen live preview on the REAL chat with Previewing bar (Cancel/Apply, matches user's reference screenshot UX). (6) 'Reduce visual effects' switch (persisted hb_chat_reduce_fx): strips wallpaper art to flat gradient, removes heavy shadows/backdrop-filter, zeroes transitions. (7) prefers-reduced-motion now also kills all chat transitions. Light+dark built-in themes 100% untouched (regression-safe). VERIFIED via own playwright screenshots: gallery opens, all 8 swatches render, love/glossy/pastel/midnight/rose/minimal previews all render beautifully, Cancel reverts + reopens gallery, Apply persists (hb_chat_theme survives reload in same browser), reduce-fx strips SVG art (computed backgroundImage = gradient only) and persists, dark theme regression OK (no data-hb-custom-theme attr), typing abcdefghijklmnopqrstuvwxyz fully responsive, tsc clean, vite prod build OK."
